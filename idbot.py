...
# -------------------- CONFIGURATION --------------------
TOKEN = '8978422983:AAHyOLqxR5zj8GGX_izFs2Spc49gnwmmK4o'
DATA_FILE = "user_data.json"

# Channel join check settings (only for /start)
ENABLE_CHANNEL_CHECK = True
REQUIRED_CHANNEL = "https://t.me/+fkKWsFmum3BhNTE1"
REQUIRED_CHANNEL_ID = -1002387714877   # ⚠️ REPLACE with actual channel ID (negative)
ADMIN_IDS = [8417161342]  # These users bypass check even on /start
...
class UserData:
    ...
    def set_verified(self, user_id, status=True):
        ...
        self.save()
    # ---------- Channel owner mapping (channel_id -> user_id) ----------
    ...
    def set_channel_owner(self, channel_id, user_id):
        ...
        self.save()
    # ---------- Channel & Group storage (with auto-update) ----------
        ...
        for ch in ud['channels']:
            if ch['id'] == channel_id:
                if ch['name'] != channel_name:
                    ...
                    self.save()
                return True  # Already exists, but updated name if changed
...
# -------------------- CHANNEL CHECK HELPERS (only for /start) --------------------
...
# ===================== CHAT MEMBER HANDLER (track bot additions) =====================
@bot.chat_member_handler()
def handle_chat_member(update):
    # update is a ChatMemberUpdated object
    chat = update.chat
    from_user = update.from_user  # user who performed the action
...
# All other commands – no membership check
...
def get_my_id(message):
    if message.chat.type == "channel":
        cid = message.chat.id; name = message.chat.title or "Channel"
    else:
        cid = message.from_user.id; name = message.from_user.first_name
...
# ==================== DIRECT SAVE FOR GROUPS AND CHANNELS ====================
# Handler for messages: /* or /Csave (channel), /** or /Gsave (group)
...
def save_group_or_channel(message):
    ...
    if chat_type in ["group", "supergroup"]:
        ...
        # Channel save – need owner
        owner_id = db.get_channel_owner(message.chat.id)
        if not owner_id:
            # If no owner mapping, fallback: set sender as owner (first user who uses command)
            owner_id = message.from_user.id
            db.set_channel_owner(message.chat.id, owner_id)
            logger.info(f"Channel {message.chat.id} owner set to {owner_id} via command fallback.")
        # Save channel for owner
        cid = message.chat.id
        ...
        # Not a group or channel
        bot.reply_to(message, "❌ This command only works in groups or channels.", parse_mode="Markdown")

# Also handle the command versions /Csave and /Gsave as fallback (they will be caught above, but we keep for safety)
# They are already handled because they match the text filter.
...
# ---------- STYLED CHANNEL ID MESSAGE (from callback) ----------
...
# ---------- STYLED GROUP ID MESSAGE (from callback) ----------
...
if __name__ == "__main__":
    print("🚀 Bot started. Channel: /* /Csave, Group: /** /Gsave with auto-save.")
    print("⚠️  REQUIRED_CHANNEL_ID is still placeholder – update it with your actual channel ID.")
